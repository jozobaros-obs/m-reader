<#
    Zostaví MD Reader do samostatného .exe pomocou PyInstalleru.
    ...
    Výstup:
        dist\MD Reader\MD Reader.exe   (režim priečinka)
        dist\MD Reader.exe             (režim -OneFile)
...
Write-Host "=== Build MD Reader .exe ===" -ForegroundColor Cyan
...
$args += (Join-Path $ScriptDir "md_reader.py")
...
$exe = if ($OneFile) { Join-Path $ScriptDir "dist\MD Reader.exe" }
       else { Join-Path $ScriptDir "dist\MD Reader\MD Reader.exe" }

Write-Host "`n=== Hotovo! ===" -ForegroundColor Green
Write-Host "Spustiteľný súbor: $exe"
Write-Host "Otvoríš ho dvojklikom, alebo cez príkaz s cestou k .md súboru."
